use godot::classes::{IArea2D, Area2D, InputEvent, Input};
...
impl Player {
    #[func]
    fn on_collision_started(&mut self, item: Gd<Area2D>) {
        godot_print!("Collision started with {:?}", item);
    }

    #[func]
    fn on_collision_end(&mut self, item: Gd<Area2D>) {
        godot_print!("Collision ended with {:?}", item);
    }
}
